...
import { quoteCommandArgument } from './utils/commandUtils.js';
import { clearSensitiveEnvironmentVariables, setTokenWasSetByTask, } from './utils/environmentUtils.js';
    ...
    try {
        ...
        if (codecovToken) {
            ...
            if (!existingToken) {
                ...
                logger.info('Environment variable CODECOV_TOKEN already has the correct value, not changing');
            }
        }
        else if (!process.env.CODECOV_TOKEN) {
        ...
        if (coverageFileName) {
            logger.info(`Coverage file name: ${coverageFileName}${!testResultFolderName ? ' (using as full path)' : ''}`);
        }
        else {
        ...
        logger.debug(`Executing command: ./codecov ${args.map((arg) => quoteCommandArgument(arg)).join(' ')}`);
