...
export function downloadFile(fileUrl, dest, options = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        fs.promises
            ...
            .catch((err) => {
            return reject(new Error(`Failed to create directory '${parentDir}': ${err.message}`));
        });
        // Define the function to set up the axios request
        const setupAxiosRequest = () => {
            // Setup axios config
            // Function to clean up on error
            const cleanup = (error, response) => {
                ...
                if (response?.data) {
                    try {
                        ...
                        logger.warn(`Warning: Failed to destroy stream: ${e instanceof Error ? e.message : String(e)}`);
                    }
                }
                    ...
                    fs.access(dest, fs.constants.F_OK, (accessErr) => {
                        if (accessErr) {
                            ...
                            fs.unlink(dest, (unlinkErr) => {
                                if (unlinkErr) {
                                    logger.warn(`Warning: Failed to clean up temporary file '${dest}': ${unlinkErr.message}`);
                                }
                                reject(error);
                            });
                        }
            ...
            axios({
                ...
                if (axios.isAxiosError(error) && error.code === 'ECONNABORTED') {
                    ...
                    cleanup(error instanceof Error ? error : new Error(String(error)), axios.isAxiosError(error) ? error.response : undefined);
                }
            });
        };
    });
 ...
 */
function getNumericHeader(headers, headerName) {
