    ...
    test('should download a file and report progress', async () => {
      ...
      await downloadFile(testUrl, testFilePath, {
        onProgress: (progress) => {
          ...
          console.log(
            `Progress: ${progress.percent}% (${progress.bytesReceived}/${progress.totalBytes} bytes)`
          );
        },
      });
    ...
    test('should handle downloads with AbortSignal', async () => {
      ...
      const downloadPromise = downloadFile(testUrl, testFilePath, {
        ...
        onProgress: (progress) => {
          progressUpdates.push({ ...progress });
          console.log(
            `Progress before abort: ${progress.percent}% (${progress.bytesReceived}/${progress.totalBytes} bytes)`
          );
        ...
        if (progressUpdates.length > 0) {
          ...
          if (lastUpdate) {
            const percentDisplay =
              lastUpdate.percent !== null ? `${lastUpdate.percent}%` : 'unknown';
